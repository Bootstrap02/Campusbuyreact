...
import { Callback } from '../../Constants/images';

const Callbacks = () => {
  const [isCheckedAll, setIsCheckedAll] = useState(false);
  const [isCheckedList, setIsCheckedList] = useState(Array(Callback.length).fill(false));

  const handleMarkSeenToggle = () => {
    const newIsCheckedAll = !isCheckedAll;
    setIsCheckedAll(newIsCheckedAll);
    setIsCheckedList(Array(Callback.length).fill(newIsCheckedAll));
  };
  ...
  return (
    <div className='container'>
      ...
      <div className='flex gap-3 m-2 px-2 py-4  max-lg:m-0 max-lg:p-0' style={{ borderBottom: '1px solid #C0C9BB' }}>
        <input type="checkbox" checked={isCheckedAll} onChange={handleMarkSeenToggle} />
        <button>Mark Seen</button>
      </div>
      <div className='flex gap-3 m-2 p-2 w-[100%]  max-lg:m-0 max-lg:p-0'>
        <div className='w-[100%]'>
          {Callback.map((notice, index) => (
